...
#----------------------------------------------------------------------------
def make_map(infile,outfile=None,paramname='blob_kT',paramweights=None,
    ...
    Input:
      ...
      infile (string):   name of file containing the 
    ...
    #----Read blob information file into a dataframe----
    df = pd.read_table(infile,sep='\t',index_col = 0)

    #----Check for weights----
    if paramweights == None:
        weights = None
    else:
    ...
    #--write history--
    history1 = ('make_map,'+infile+',outfile='+nstr(outfile)+',paramname='
                +nstr(paramname)+',paramweights='+nstr(paramweights)
